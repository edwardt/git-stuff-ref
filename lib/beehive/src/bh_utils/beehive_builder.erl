...
start() ->
  ...
  ScratchDisk =
    config:search_for_application_value(scratch_dir, "/tmp/beehive/scratch"),
  SquashedDisk =
    config:search_for_application_value(squashed_disk,
                                        "/tmp/beehive/squashed"),
  BeeImage =
    filename:join([filename:absname(SquashedDisk),
                   App#app.name,
                   lists:append([App#app.name, ".bee"])]),
  HostIp = bh_host:myip(),
...
parse_args([ "-h" | _Rest], _Params) ->
  show_usage();
parse_args([ "-a", AppFile | Rest], Params) ->
  parse_args(Rest, Params#params{app_file = AppFile});
parse_args([ "-d", Dir | Rest], Params) ->
  parse_args(Rest, Params#params{root_dir = Dir});
parse_args([ "-p", Port | Rest], Params) ->
  parse_args(Rest, Params#params{port = list_to_integer(Port)});
