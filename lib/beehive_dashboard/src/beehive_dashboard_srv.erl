...
client_disconnected(WebSocket) ->
  gen_server:cast(?SERVER, {client_disconnected, WebSocket}).

send_message_to_all_websockets(Msg) ->
  gen_server:cast(?SERVER, {send_message_to_all_websockets, Msg}).
...
start_web_server(#state{docroot = DocRoot, web_port = Port} = _State) ->
  ...
  WebConfig = [{port, Port},
               {name, ?MODULE},
               {loop, Loop},
               {ws_loop, WebSocketLoop}],
  misultin:start_link(WebConfig).

%% Loops
% Web requests
handle_web_req(Req, Docroot) ->
  handle_web(Req:get(method),
             Req:resource([lowercase, urldecode]), Req, Docroot).

handle_web('GET', [], Req, Docroot) ->
  serve_file(["index.html"], Req, Docroot);
